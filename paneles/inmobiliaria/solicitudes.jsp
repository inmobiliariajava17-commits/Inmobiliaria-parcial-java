...
<main class="page-wrap"><div class="container app-container">
    ...
    <div class="surface-card mb-4"><div class="d-flex flex-wrap gap-2">
        <a class="btn btn-sm <%= request.getParameter("estado")==null ? "btn-primary":"btn-outline-primary" %>" href="<%=request.getContextPath()%>/acciones/solicitudes-inmobiliaria.jsp">Todas</a>
        <a class="btn btn-sm <%= "EN_REVISION".equals(request.getParameter("estado")) ? "btn-primary":"btn-outline-primary" %>" href="<%=request.getContextPath()%>/acciones/solicitudes-inmobiliaria.jsp?estado=EN_REVISION">En revisi&#243;n</a>
        <a class="btn btn-sm <%= "APROBADA".equals(request.getParameter("estado")) ? "btn-primary":"btn-outline-primary" %>" href="<%=request.getContextPath()%>/acciones/solicitudes-inmobiliaria.jsp?estado=APROBADA">Aprobadas</a>
        <a class="btn btn-sm <%= "RECHAZADA".equals(request.getParameter("estado")) ? "btn-primary":"btn-outline-primary" %>" href="<%=request.getContextPath()%>/acciones/solicitudes-inmobiliaria.jsp?estado=RECHAZADA">Rechazadas</a>
        <a class="btn btn-sm <%= "BORRADOR".equals(request.getParameter("estado")) ? "btn-primary":"btn-outline-primary" %>" href="<%=request.getContextPath()%>/acciones/solicitudes-inmobiliaria.jsp?estado=BORRADOR">Borradores</a>
    </div></div>
    ...
    <% } else { %>
        ...
        %>
            <div class="col-lg-6 align-self-start" id="solicitud-<%=s.get("id")%>"><div class="surface-card">
                ...
                <% if (!seleccionada) { %>
                    <a class="btn btn-primary btn-sm" href="<%=request.getContextPath()%>/acciones/solicitudes-inmobiliaria.jsp?idSolicitud=<%=s.get("id")%>#solicitud-<%=s.get("id")%>">Ver solicitud</a>
                <% } else { %>
                    <div class="border-top pt-3 mt-2">
                        <div class="d-flex justify-content-between align-items-start gap-3 mb-3">
                            <div><div class="eyebrow">Detalle</div><h3 class="h5 fw-bold mb-0">Informaci&#243;n de la solicitud</h3></div>
                            <a class="btn btn-outline-secondary btn-sm" href="<%=request.getContextPath()%>/acciones/solicitudes-inmobiliaria.jsp#solicitud-<%=s.get("id")%>">Cerrar</a>
                        </div>
