...
private void cargarDetalle(HttpServletRequest request, int idSolicitud, int idUsuario) throws SQLException {
        ...
        try (Connection con = obtenerConexion(); PreparedStatement ps = con.prepareStatement(docSql)) {
            ps.setInt(1, idSolicitud);
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
...
if (!esInmobiliaria(session)) {
        ...
        try {
            ...
            response.sendRedirect(request.getContextPath() + "/acciones/solicitudes-inmobiliaria.jsp?error=datos");
            return;
...
if (!esInmobiliaria(session)) {
        ...
        try {
            ...
            if ("aprobar".equals(accion)) {
                ...
                response.sendRedirect(request.getContextPath() + "/acciones/solicitudes-inmobiliaria.jsp?error=accion");
                return;
            }

            cambiarEstado(idSolicitud, idUsuario, nuevoEstado, textoAuditoria);
            response.sendRedirect(request.getContextPath() + "/acciones/solicitudes-inmobiliaria.jsp?idSolicitud=" + idSolicitud + "&mensaje=" + accion);
        } catch (NumberFormatException e) {
            response.sendRedirect(request.getContextPath() + "/acciones/solicitudes-inmobiliaria.jsp?error=datos");
        } catch (SQLException e) {
            response.sendRedirect(request.getContextPath() + "/acciones/solicitudes-inmobiliaria.jsp?error=guardar");
        }
