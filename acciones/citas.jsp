...
private void agendar(HttpServletRequest request, HttpServletResponse response, int idUsuario)
        ...
        if (idPropiedadTexto == null || fechaTexto == null || horaTexto == null
            ...
            response.sendRedirect(request.getContextPath() + "/acciones/citas.jsp?error=datos");
            return;
        }
        ...
        try {
            ...
            response.sendRedirect(request.getContextPath() + "/acciones/detalle-propiedad.jsp?id=" + idPropiedad + "&cita=fecha");
            return;
        }

        if (!esHorarioValido(fechaHora.toLocalTime())) {
            response.sendRedirect(request.getContextPath() + "/acciones/detalle-propiedad.jsp?id=" + idPropiedad + "&cita=horario");
            return;
        }

        if (!fechaHora.isAfter(LocalDateTime.now())) {
            response.sendRedirect(request.getContextPath() + "/acciones/detalle-propiedad.jsp?id=" + idPropiedad + "&cita=pasada");
            return;
        }
        ...
        try (Connection con = obtenerConexion()) {
            try (PreparedStatement ps = con.prepareStatement(verificarPropiedad)) {
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    if (!rs.next()) {
                        response.sendRedirect(request.getContextPath() + "/acciones/detalle-propiedad.jsp?id=" + idPropiedad + "&cita=no-disponible");
                        return;
                    }
                }
            }
        ...
        response.sendRedirect(request.getContextPath() + "/acciones/citas.jsp?mensaje=agendada");
    }
...
private void cancelar(HttpServletRequest request, HttpServletResponse response, int idUsuario)
        ...
        try (Connection con = obtenerConexion();
            ...
            int filas = ps.executeUpdate();
            if (filas == 0) {
                response.sendRedirect(request.getContextPath() + "/acciones/citas.jsp?error=cancelar");
                return;
            }
        }

        registrarAuditoria(idUsuario, "Canceló la cita: " + idCita);
        response.sendRedirect(request.getContextPath() + "/acciones/citas.jsp?mensaje=cancelada");
    }
...
if (!esCliente(session)) {
        ...
        try {
            if ("agendar".equals(accion)) {
                ...
                response.sendRedirect(request.getContextPath() + "/acciones/citas.jsp?error=accion");
            }
        } catch (NumberFormatException e) {
            response.sendRedirect(request.getContextPath() + "/acciones/citas.jsp?error=datos");
        } catch (SQLException e) {
            response.sendRedirect(request.getContextPath() + "/acciones/citas.jsp?error=guardar");
        }
